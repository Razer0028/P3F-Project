...
$configPath = '/opt/serveradmin/config/portal_services.json';
$resultMsg = '';
...
$monitorSelected = $portalConfig['monitor_containers'] ?? [];
if (!is_array($monitorSelected)) {
    $monitorSelected = [];
}
...
foreach ($services as $service) {
    ...
    if (is_string($name) && $name !== '') {
        $knownContainers[$name] = true;
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!hash_equals($_SESSION['csrf'], $_POST['csrf'] ?? '')) {
        $resultMsg = '❌ CSRFトークンが無効です。';
    } elseif (!is_writable($configPath) && !is_writable(dirname($configPath))) {
        $resultMsg = '❌ 設定ファイルを書き込めません。/opt/serveradmin/config の権限とマウント設定を確認してください。';
    } else {
        $enabledRaw = $_POST['enabled'] ?? [];
        $monitorRaw = $_POST['monitor'] ?? [];
        $autoStopEnabled = isset($_POST['auto_stop_enabled']);

        $enabled = [];
        if (is_array($enabledRaw)) {
            foreach ($enabledRaw as $id) {
                if (is_string($id) && in_array($id, $serviceIds, true)) {
                    $enabled[] = $id;
                }
            }
        }
        ...
        $monitor = [];
        if (is_array($monitorRaw)) {
            foreach ($monitorRaw as $name) {
                if (is_string($name) && isset($knownContainers[$name])) {
                    $monitor[] = $name;
                }
            }
        }
        ...
        if (!isset($current['services']) || !is_array($current['services'])) {
            $current['services'] = $services;
        }

        $current['enabled'] = array_values(array_unique($enabled));
        $current['monitor_containers'] = array_values(array_unique($monitor));
        $current['auto_stop_enabled'] = (bool)$autoStopEnabled;
        if (!isset($current['auto_stop_minutes']) || intval($current['auto_stop_minutes']) <= 0) {
            $current['auto_stop_minutes'] = $autoStopMinutes;
        }

        $payload = json_encode($current, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        if ($payload === false) {
            $resultMsg = '❌ JSONの生成に失敗しました。';
        } else {
            $ok = file_put_contents($configPath, $payload . "\n", LOCK_EX);
            if ($ok === false) {
                $resultMsg = '❌ 設定ファイルの保存に失敗しました。';
            } else {
                $resultMsg = '✅ 設定を保存しました。';
                $portalConfig = portal_load_config();
                $services = $portalConfig['services'] ?? [];
...
<div class="card">
  <form method="post">
    ...
    <div class="toggle-card">
      <div>
        <div class="section-title">自動停止</div>
        <div class="note">0人状態が<?= h($autoStopMinutes) ?>分続くと停止</div>
      </div>
      <label class="switch">
        <input type="checkbox" name="auto_stop_enabled" value="1" <?= $autoStopEnabled ? 'checked' : '' ?>>
        <span class="slider"></span>
      </label>
    </div>
    <div class="note">無効でもプレイヤー数の計測は継続されます。</div>
    ...
    <table class="table">
      ...
      <tbody>
        ...
        <tr>
          <td>
            <input class="checkbox" type="checkbox" name="enabled[]" value="<?= h($id) ?>" <?= is_checked($id, $enabledIds) ?>>
          </td>
          <td>
            <?php if (is_string($container) && $container !== ''): ?>
    ...
    <div class="action-row">
      ...
      <div class="note">保存後はログビューア/管理パネルの表示対象が更新されます。</div>
    </div>
  </form>
</div>
